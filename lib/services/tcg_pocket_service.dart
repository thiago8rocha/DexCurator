...
const String _kFlibCards = 'https://raw.githubusercontent.com/flibustier/pokemon-tcg-pocket-database/main/dist/cards';
const String _kFlibImgs  = 'https://raw.githubusercontent.com/flibustier/pokemon-tcg-exchange/main/public/images/cards-by-set';
...
// Sets com imagem PNG (demais são WebP)
const Set<String> kPngBoosters = {'B2b'};

String boosterAssetPath(String id) =>
    'assets/pocket/boosters/$id.${kPngBoosters.contains(id) ? "png" : "webp"}';
...
const Map<String, PocketSetMeta> kPocketSetMeta = {
  ...
  'A2b': PocketSetMeta(id: 'A2b', namePt: 'Festival Brilhante',        releaseDate: '2025-04-30', color1: 0xFF3D0020, color2: 0xFF003040),
  ...
  'A3b': PocketSetMeta(id: 'A3b', namePt: 'Bosque dos Eevee',          releaseDate: '2025-05-29', color1: 0xFF0A2E0A, color2: 0xFF280040),
  'A4':  PocketSetMeta(id: 'A4',  namePt: 'Sabedoria dos Céus e Mares',releaseDate: '2025-07-03', color1: 0xFF002040, color2: 0xFF003035),
  'A4a': PocketSetMeta(id: 'A4a', namePt: 'Nascentes Reclusas',        releaseDate: '2025-08-21', color1: 0xFF062010, color2: 0xFF002035),
  'A4b': PocketSetMeta(id: 'A4b', namePt: 'Pack Deluxe ex',            releaseDate: '2025-09-18', color1: 0xFF3D1000, color2: 0xFF1A1400),
  'B1':  PocketSetMeta(id: 'B1',  namePt: 'Mega Ascensão',             releaseDate: '2025-10-16', color1: 0xFF2D0010, color2: 0xFF190030),
  'B1a': PocketSetMeta(id: 'B1a', namePt: 'Chama Carmesim',            releaseDate: '2025-12-17', color1: 0xFF3D0000, color2: 0xFF3D1800),
...
class PocketCardBrief {
  ...
  const PocketCardBrief({
    ...
    this.imageUrlLow, this.rarity, this.boosters = const [],
  });

  factory PocketCardBrief.fromJson(Map<String, dynamic> json) {
    final raw = json['image'] as String?;
    final rawBoosters = json['boosters'] as List<dynamic>?;
    return PocketCardBrief(
      ...
      boosters:    rawBoosters?.map((e) => e.toString()).toList() ?? const [],
    );
  }
}
...
class TcgPocketService {
  ...
  static Future<List<PocketSet>> fetchSeries() async {
    ...
    try {
      ...
      // Adiciona sets conhecidos não retornados pela API (lançados mas sem dados no TCGdex ainda)
  ...
  static List<PocketSet> localSeries() => _fallbackSeries();
  ...
  // Converte IDs de set do app para IDs do flibustier
  static String _flibSetId(String setId) => switch (setId) {
    'P-A' => 'PROMO-A',
    'P-B' => 'PROMO-B',
    _     => setId,
  };
  ...
  static Future<PocketSet?> fetchSet(String setId) async {
    ...
    try {
      final res = await http
          .get(Uri.parse('$_kBase/sets/$setId'), headers: _headers)
          .timeout(_timeout);
    ...
    // 3. Fallback flibustier quando TCGdex não tem imagens
    final missingImages = set == null ||
        (set.cards.isNotEmpty && set.cards.every((c) => c.imageUrlLow == null));

    if (missingImages) {
      final flib = await _fetchFlibustierSet(setId);
      if (flib != null) {
        set = (set != null && set.cards.isNotEmpty)
            ? _patchImages(set, flib)
            : flib;
      }
    }
    ...
    if (set != null) {
      _setCache[setId] = set;
      _persistSet(setId, set);
    }
    return set;
  }

  /// Busca dados do set via flibustier/pokemon-tcg-pocket-database.
  static Future<PocketSet?> _fetchFlibustierSet(String setId) async {
    try {
      final fid = _flibSetId(setId);
      final res = await http
          .get(Uri.parse('$_kFlibCards/$fid.json'), headers: _headers)
          .timeout(_timeout);
      if (res.statusCode != 200) return null;

      final rawList = jsonDecode(res.body) as List<dynamic>;
      final cards = rawList.map((c) {
        final m       = c as Map<String, dynamic>;
        final number  = (m['number'] as num).toInt();
        final localId = number.toString().padLeft(3, '0');
        final rawPacks = m['packs'] as List<dynamic>?;
        return PocketCardBrief(
          id:          '$setId-$localId',
          localId:     localId,
          name:        m['name'] as String,
          imageUrlLow: '$_kFlibImgs/$fid/$number.webp',
          rarity:      m['rarity']?.toString(),
          boosters:    rawPacks?.map((e) => e.toString()).toList() ?? const [],
        );
      }).toList()
        ..sort((a, b) {
          final an = int.tryParse(a.localId) ?? 9999;
      ...
      // Conta apenas cartas sem SSR/UR como total oficial (equivale ao cardCount.official do TCGdex)
      const _extraRarities = {'ssr', 'ur'};
      final officialCount = rawList.where((c) {
        final r = ((c as Map<String, dynamic>)['rarity']?.toString() ?? '').toLowerCase();
        return !_extraRarities.contains(r);
      }).length;

      final meta = kPocketSetMeta[setId];
      return PocketSet(
        id: setId, name: meta?.namePt ?? setId,
        releaseDate: meta?.releaseDate,
        totalCards: officialCount, cards: cards,
      );
    } catch (_) { return null; }
  }

  /// Copia as imagens do flibustier para cards do TCGdex que não têm imagem.
  static PocketSet _patchImages(PocketSet tcgdex, PocketSet flib) {
    final flibMap = {for (final c in flib.cards) c.localId: c};
    final patched = tcgdex.cards.map((c) {
      final fb = flibMap[c.localId];
      if (c.imageUrlLow != null && fb == null) return c;
      return PocketCardBrief(
        id: c.id, localId: c.localId, name: c.name,
        imageUrlLow: c.imageUrlLow ?? fb?.imageUrlLow,
        rarity: c.rarity ?? fb?.rarity,
        boosters: c.boosters.isNotEmpty ? c.boosters : (fb?.boosters ?? const []),
      );
    }).toList();
    return PocketSet(id: tcgdex.id, name: tcgdex.name, logoUrl: tcgdex.logoUrl,
        releaseDate: tcgdex.releaseDate, totalCards: tcgdex.totalCards, cards: patched);
  }

  // v3: URL de imagem completa no cache (v2 stripava /low.webp, incompatível com flibustier)
  static const _kCachePrefix = '_pocket_set_v3_';

  /// Carrega um set do cache persistente.
  static Future<PocketSet?> _loadCachedSet(String setId) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final raw   = prefs.getString('$_kCachePrefix$setId');
      if (raw == null) return null;
      final json    = jsonDecode(raw) as Map<String, dynamic>;
      final cardList = (json['cards'] as List<dynamic>?) ?? [];
      final cards = cardList.map((c) {
        final m = c as Map<String, dynamic>;
        final rawBoosters = m['boosters'] as List<dynamic>?;
        return PocketCardBrief(
          id:          m['id']      as String,
          localId:     m['localId'] as String,
          name:        m['name']    as String,
          imageUrlLow: m['imageUrl'] as String?,
      ...
      if (cards.isNotEmpty && cards.every((c) => c.imageUrlLow == null)) return null;
      final meta = kPocketSetMeta[setId];
      return PocketSet(
        id: json['id'] as String,
        name: meta?.namePt ?? json['name'] as String,
        totalCards: json['totalCards'] as int? ?? cards.length,
        cards: cards,
      );
    } catch (_) { return null; }
  }

  /// Persiste um set no cache local (sem bloquear).
  static void _persistSet(String setId, PocketSet set) {
    if (set.cards.isEmpty || set.cards.every((c) => c.imageUrlLow == null)) return;
    SharedPreferences.getInstance().then((prefs) {
      prefs.setString('$_kCachePrefix$setId', jsonEncode({
        'id':         set.id,
        'name':       set.name,
  ...
  /// Usa a lista real da API (via fetchSeries) para não tentar sets inexistentes.
  static void warmupRecentSets({int count = 3}) {
    Future(() async {
      final series = await fetchSeries();
      final recent = series
          .where((s) => !s.id.startsWith('P-'))
          .take(count)
          .toList();
      for (final s in recent) {
        if (_setCache.containsKey(s.id)) continue;
        await fetchSet(s.id);
