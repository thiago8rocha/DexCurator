...
class StorageService {
  ...
  static const String _formCaughtPrefix = 'caught_nacional_f';
  ...
  /// Pré-inicializa o SharedPreferences. Chamar no main() antes de runApp()
  /// para que o primeiro acesso em _initPokedex não cause jank.
  ...
  // ─── CAPTURA DE FORMAS NACIONAIS ───────────────────────────────
  // Chave separada para não inflar o contador da home screen.

  Future<bool> isFormCaught(int formId) async {
    final prefs = await _instance;
    return prefs.getBool('$_formCaughtPrefix$formId') ?? false;
  }

  Future<void> setFormCaught(int formId, bool caught) async {
    final prefs = await _instance;
    await prefs.setBool('$_formCaughtPrefix$formId', caught);
  }

  /// Versão com suporte a formaKey e formId — usada pela PokedexScreen.
  Future<Map<String, bool>> getCaughtMapByEntries(
    ...
    for (final e in entries) {
      ...
      final catchKey = e['catchKey'] as String? ??
          (e['formaKey'] != null
              ? '${speciesId}_${e['formaKey']}'
              : '$speciesId');
      final formId = e['formId'] as int?;
      if (formId != null) {
        map[catchKey] = prefs.getBool('$_formCaughtPrefix$formId') ?? false;
      } else {
        map[catchKey] =
            prefs.getBool('${_caughtPrefix}${pokedexId}_$speciesId') ?? false;
      }
    }
    return map;
  }
