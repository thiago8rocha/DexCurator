...
import 'package:dexcurator/core/pokemon_types.dart' show typeColor, typeTextColor;
import 'package:dexcurator/screens/detail/detail_shared.dart'
    show ptType, typeIconAsset, neutralBg, kPokeApiBase, PokeballLoader, BilingualTerm, bilingualModeNotifier;
...
import 'package:dexcurator/screens/menu/move_detail_screen.dart' show MoveDetailSheet;
...
class MoveEntry {
  ...
  String    flavor       = '';
  List<int> pokemonIds   = [];
  Map<String, List<int>> methodGroups = {};
...
class _MovesListScreenState extends State<MovesListScreen> {
  ...
  Future<void> _loadMoves() async {
    ...
    // Carregar move_map.json e move_methods.json em paralelo
    final results = await Future.wait<dynamic>([
      _tryLoadMoveMap(),
      _tryLoadMethodMap(),
    ]);
    final localMap = results[0] as Map<String, dynamic>?;
    if (localMap != null) {
  ...
  // Carrega move_methods.json uma vez e cacheia
  static Map<String, dynamic>? _methodMapData;
  Future<void> _tryLoadMethodMap() async {
    if (_methodMapData != null) return;
    try {
      final raw = await rootBundle.loadString('assets/data/move_methods.json');
      _methodMapData = jsonDecode(raw) as Map<String, dynamic>;
    } catch (_) {}
  }
  ...
  Future<void> _loadFromMoveMap(Map<String, dynamic> localMap) async {
    ...
    for (final kv in localMap.entries) {
      ...
      final e = MoveEntry(nameEn: nameEn, url: '$kPokeApiBase/move/$nameEn');
      e.typeEn     = data['type']   as String? ?? '';
      e.category   = data['cat']    as String? ?? '';
      e.power      = data['power']  as int?;
      e.accuracy   = data['acc']    as int?;
      e.pp         = data['pp']     as int?;
      e.effect     = data['effectPt'] as String? ?? '';
      e.flavor     = data['flavorPt'] as String? ?? '';
      e.pokemonIds = gamePoke;

      // Grupos por método de aprendizado
      final methodRaw = (_methodMapData?[nameEn] as Map<String, dynamic>?) ?? {};
      final groups = <String, List<int>>{};
      for (final mEntry in methodRaw.entries) {
        final method  = mEntry.key as String;
        final allIds  = (mEntry.value as List<dynamic>).cast<int>();
        final filtered = gameIds.isEmpty
            ? allIds
            : allIds.where((id) => gameIds.contains(id)).toList();
        if (filtered.isNotEmpty) groups[method] = filtered;
      }
      e.methodGroups = groups;
          ...
          Padding(
            ...
            child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Expanded(child: BilingualTerm(
                nameEn: entry.nameEn,
                namePt: translateMove(entry.nameEn),
                baseStyle: const TextStyle(fontSize: 14, fontWeight: FontWeight.w600),
                secondaryStyle: const TextStyle(fontSize: 11),
              )),
              _Stat('Poder',    entry.power    != null ? '${entry.power}'     : '—'),
              const SizedBox(width: 16),
              _Stat('Precisão', entry.accuracy != null ? '${entry.accuracy}%' : '—'),
              const SizedBox(width: 16),
              ...
              if (typeEn.isNotEmpty)
                Container(
                  ...
                  child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                    Image.asset(typeIconAsset(typeEn), width: 12, height: 12,
                        errorBuilder: (_, __, ___) => const SizedBox(width: 12)),
                    const SizedBox(width: 2),
                    Text(typePt, style: const TextStyle(fontSize: 8,
                        fontWeight: FontWeight.w700, color: Colors.white)),
                  ]),
                )
              else
                Container(width: 64, height: 16,
