...
class ItemEntry {
  ...
  final Map<String, String> pocketByGame;
  final int                 ballOrder;
  ...
  const ItemEntry({
    ...
    required this.effectEn, required this.flavorEn,
    required this.sprite, required this.versions,
    this.pocketByGame = const {},
    this.ballOrder    = 999,
  });

  String pocketFor(String gameId) =>
      pocketByGame[gameId] ?? pocket;
  ...
  // Para MTs/MOs: extrai o nome do golpe do effectEn
  // "Teaches Flamethrower to a compatible Pokémon." -> "Flamethrower"
  String? get moveName {
    if (category != 'all-machines' && category != 'machines') return null;
    final m = RegExp(r'Teaches (.+?) to a').firstMatch(effectEn);
    return m?.group(1);
  }

  // Descrição para a lista
  String get shortDesc => effectEn.isNotEmpty ? effectEn : flavorEn;

  // Sprite: local (asset) ou rede
  bool get isSpriteLocal => sprite.startsWith('assets/');
...
const _pocketIcon = {
  ...
  'Captura':          Icons.catching_pokemon,
  'Medicina':         Icons.medical_services_outlined,
  'Medicamentos':     Icons.medical_services_outlined,
  'MTs e MOs':        Icons.disc_full_outlined,
  'MTs':              Icons.disc_full_outlined,
  'Frutas':           Icons.eco_outlined,
  'Berry Pouch':      Icons.eco_outlined,
  'Itens de Batalha': Icons.bolt_outlined,
  'Batalha':          Icons.bolt_outlined,
  'Cartas':           Icons.mail_outlined,
  'Outros Itens':     Icons.inventory_2_outlined,
  'Bolsa':            Icons.backpack_outlined,
...
// ─── Função de agrupamento (roda em isolate) ──────────────────────
Map<String, List<ItemEntry>> _buildGroupedIsolate(Map<String, dynamic> args) {
  ...
  final entries = map.entries.map((e) {
    ...
    return ItemEntry(
      ...
      effectEn:   v['effectEn']   as String? ?? v['effect'] as String? ?? '',
      flavorEn:   v['flavorEn']   as String? ?? v['flavor'] as String? ?? '',
      sprite:      v['sprite']     as String? ?? '',
      versions:    (v['versions'] as List<dynamic>?)
          ?.cast<String>()
          .where((s) => s.isNotEmpty)
          .toList() ?? [],
      pocketByGame: (v['pocketByGame'] as Map<String, dynamic>?)
          ?.map((k, val) => MapEntry(k, val as String)) ?? const {},
      ballOrder:   (v['ballOrder'] as num?)?.toInt() ?? 999,
    );
  }).where((item) {
    if (skipCategories.contains(item.categoryPt)) return false;
    if (gameId != null) {
      // Se versions está preenchido, filtra pelo jogo ativo
      if (item.versions.isNotEmpty) {
        return item.versions.contains(gameId);
      }
      // Itens sem versions (chave, evento) só aparecem no modo "todos os jogos"
      return false;
    }
    return true;
  }).where((item) {
  ...
  // Agrupar pela aba do jogo ativo (pocketFor) em vez de categoryPt genérico
  final gId = gameId ?? '';
  final grouped = <String, List<ItemEntry>>{};
  for (final item in entries) {
    final pocket = item.pocketFor(gId);
    (grouped[pocket] ??= []).add(item);
  }
  for (final key in grouped.keys) {
    grouped[key]!.sort((a, b) {
      // Pokébolas: ordenar por ballOrder
      if (a.ballOrder != 999 || b.ballOrder != 999) {
        return a.ballOrder.compareTo(b.ballOrder);
      }
      // MTs/MOs: tm01...tm229, tr00...tr99, hm01...
      if (a.category == 'all-machines' && b.category == 'all-machines') {
        return _machineSort(a.nameEn).compareTo(_machineSort(b.nameEn));
      }
...
int _machineSort(String name) {
  final m = RegExp(r'^(tm|tr|hm)(\d+)$').firstMatch(name);
  if (m == null) return 99999;
  final num = int.parse(m.group(2)!);
  if (m.group(1) == 'tm') return num;
  if (m.group(1) == 'tr') return 20000 + num;
  return 30000 + num;
}
...
class _ItemsListScreenState extends State<ItemsListScreen> {
  ...
  Future<void> _init() async {
    ...
    await _rebuildGrouped();
    if (mounted) setState(() => _loading = false);
  }

  Future<void> _rebuildGrouped() async {
    final result = await compute(_buildGroupedIsolate, {
      'map':        _itemMapCache ?? {},
      'gameId':     _filterByGame ? _activeGameId : null,
      'search':     _search,
    });
    _grouped = result;
  }

  void _changeGame(Map<String, dynamic> g) async {
    setState(() {
  ...
  List<String> get _sortedCategories {
    ...
    cats.sort((a, b) {
      final ia = _pocketOrder.indexOf(a);
      final ib = _pocketOrder.indexOf(b);
              ...
              GestureDetector(
                ...
                child: Container(
                  ...
                  child: Row(children: [
                    ...
                    GestureDetector(
                      onTap: () async {
                        setState(() => _filterByGame = !_filterByGame);
                        await _rebuildGrouped();
                        if (mounted) setState(() {});
                      },
                      child: Container(
      ...
      child: Container(
        ...
        child: Row(crossAxisAlignment: CrossAxisAlignment.start,
          ...
          Expanded(child: Column(
            ...
            if (item.moveName == null && item.shortDesc.isNotEmpty) ...[
...
class ItemSprite extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Sprite local (bundlado após download_item_sprites.py)
    ...
    // Fallback rede (itens sem sprite local ainda)
    return Image(
      key: ValueKey(url),
      image: ResizeImage(NetworkImage(url),
          width: (size * 2).toInt(), policy: ResizeImagePolicy.fit),
      width: size, height: size,
      fit: BoxFit.contain,
