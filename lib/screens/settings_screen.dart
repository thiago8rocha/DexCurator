...
import 'package:dexcurator/screens/detail/detail_shared.dart'
    show bilingualModeNotifier, defaultSpriteNotifier, showFormsInListNotifier, PokeballLoader;
      ...
      body: ListView(
        children: [
          ...
          _SettingsTile(
            ...
            title: 'Formas na Pokédex Nacional',
            subtitle: 'Exibir formas alternativas como entradas separadas',
            onTap: () => Navigator.push(context,
              MaterialPageRoute(builder: (_) => const FormsInListSettingsScreen())),
          ),
...
// ─── FORMAS NA POKÉDEX NACIONAL ──────────────────────────────────

class FormsInListSettingsScreen extends StatefulWidget {
  const FormsInListSettingsScreen({super.key});
  @override
  State<FormsInListSettingsScreen> createState() => _FormsInListSettingsScreenState();
}

class _FormsInListSettingsScreenState extends State<FormsInListSettingsScreen> {
  final _storage = StorageService();
  bool _enabled = true;
  ...
  void initState() {
    super.initState();
    _enabled = showFormsInListNotifier.value;
  }

  Future<void> _toggle(bool val) async {
    setState(() => _enabled = val);
    await _storage.setShowFormsInList(val);
    showFormsInListNotifier.value = val;
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Formas na Pokédex Nacional'),
        scrolledUnderElevation: 0,
        surfaceTintColor: Colors.transparent,
      ),
      body: ListView(
        children: [
          Padding(
            ...
            child: Container(
              ...
              child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                const SizedBox(width: 8),
                Expanded(child: Text(
                  'Quando ativado, formas alternativas (Mega, Gigantamax, regionais, etc.) '
                  'aparecem como entradas independentes na Pokédex Nacional, com rastreamento de captura próprio. '
                  'Não afeta outras Pokédex.',
          ...
          SwitchListTile(
            title: const Text('Exibir formas na lista'),
            subtitle: const Text('Formas alternativas com sprite próprio'),
            value: _enabled,
            onChanged: _toggle,
          ),
        ],
