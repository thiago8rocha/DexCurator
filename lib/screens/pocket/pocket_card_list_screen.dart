...
import 'package:dexcurator/screens/pocket/pocket_image_loader.dart';
import 'package:dexcurator/core/app_constants.dart' show kTcgDexAssets;
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _set     = set;
          _loading = false;
          if (set == null) _error = 'Cartas ainda não disponíveis.\nEsta coleção foi lançada recentemente e seus dados ainda não estão disponíveis. Tente novamente em breve.';
        });
          ...
          children: [
            ...
            Expanded(
              child: ClipRRect(
                ...
                child: PocketNetworkImage(
                        url: card.imageUrlLow ?? '$kTcgDexAssets/tcgp/$setId/${card.localId}/low.webp',
                        fit: BoxFit.cover,
                        loading: Container(color: scheme.surfaceContainerHighest),
                        error:   _CardPlaceholder(scheme: scheme),
                      ),
              ),
            ),
