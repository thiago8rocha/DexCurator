...
class PocketRarityBadge extends StatelessWidget {
  ...
  List<Widget> _buildIcons(String rarity, ColorScheme scheme) {
    ...
    // Códigos curtos (flibustier)
    if (r == 'c')              return _diamonds(1);
    if (r == 'u')              return _diamonds(2);
    if (r == 'r')              return _diamonds(3);
    if (r == 'rr')             return _diamonds(4);
    if (r == 'ar')             return _stars(1);
    if (r == 'sr' || r == 'sar') return _stars(2);
    if (r == 'im')             return _stars(3);
    if (r == 's')              return _shinies(1);
    if (r == 'ssr')            return _shinies(2);
    if (r == 'ur')             return [_crown()];
